...
C     
C     Process: g u~ > t~ z d~ b WEIGHTED=7
C     *   Decay: t~ > s c~ b~ WEIGHTED=4
C     *   Decay: z > s s~ WEIGHTED=2
C     Process: g c~ > t~ z s~ b WEIGHTED=7
C     *   Decay: t~ > s c~ b~ WEIGHTED=4
C     *   Decay: z > s s~ WEIGHTED=2
C     
      IMPLICIT NONE
...
C           Set right sign for ANS, based on sign of chosen helicity
 ...
 10     CONTINUE
      ...
      REAL*8 FUNCTION MATRIX18(P,NHEL,IC)
C     
C     Generated by MadGraph5_aMC@NLO v. 2.2.1, 2014-09-25
C     By the MadGraph5_aMC@NLO Development Team
C     Visit launchpad.net/madgraph5 and amcatnlo.web.cern.ch
...
C     1 T(1,2,8) T(3,4) T(6,7) T(9,5)
      DATA DENOM(2)/1/
      DATA (CF(I,  2),I=  1,  2) /    0,  108/
C     1 T(1,9,5) T(2,8) T(3,4) T(6,7)
C     ----------
C     BEGIN CODE
C     ----------
...
C     Amplitude(s) for diagram number 5
      ...
      CALL FFV2_3_2(W(1,5),W(1,3),GC_50,GC_58,ZERO,ZERO,W(1,12))
     ...
     $ +AMP(18)+AMP(19)+AMP(20)+AMP(22)+AMP(23)
      ...
      DO M = 1, NAMPSO
        DO I = 1, NCOLOR
          ...
          ENDDO
          DO N = 1, NAMPSO
            IF (CHOSEN_SO_CONFIGS(SQSOINDEX18(M,N))) THEN
              MATRIX18 = MATRIX18 + ZTEMP*DCONJG(JAMP(I,N))/DENOM(I)
            ENDIF
          ENDDO
        ENDDO
      ENDDO
