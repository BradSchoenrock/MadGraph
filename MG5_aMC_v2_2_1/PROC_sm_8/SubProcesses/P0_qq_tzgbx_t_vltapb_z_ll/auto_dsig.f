...
C       Output weights and number of events
        ...
        DO J=1,SYMCONF(0)
          WRITE(*,'(12E12.4)')((SUMWGT(K,I,J)/SUMPROB,K=1,2),I=1
     $     ,MAXSPROC)
        ENDDO
        SUMPROB=0D0
        DO J=1,SYMCONF(0)
        ...
        DO J=1,SYMCONF(0)
          WRITE(*,'(12E12.4)')((NUMEVTS(K,I,J)/SUMPROB,K=1,2),I=1
     $     ,MAXSPROC)
        ENDDO
        WRITE(*,*)'Events:'
        DO J=1,SYMCONF(0)
          WRITE(*,'(12I12)')((NUMEVTS(K,I,J),K=1,2),I=1,MAXSPROC)
        ENDDO
...
C     EXTERNAL FUNCTIONS
C     
      DOUBLE PRECISION DSIG1,DSIG2,DSIG3,DSIG4,DSIG5,DSIG6
      LOGICAL PASSCUTS
C     
C     LOCAL VARIABLES 
C     
...
C       Flip CM_RAP (to get rapidity right)
      ...
      IF (PASSCUTS(P1)) THEN
        IF(IPROC.EQ.1) DSIGPROC=DSIG1(P1,WGT,IMODE)  ! u d~ > vt ta+ b e- e+ g b~
        IF(IPROC.EQ.2) DSIGPROC=DSIG2(P1,WGT,IMODE)  ! u d~ > vt ta+ b ve ve~ g b~
        IF(IPROC.EQ.3) DSIGPROC=DSIG3(P1,WGT,IMODE)  ! u d~ > vt ta+ b vt vt~ g b~
        IF(IPROC.EQ.4) DSIGPROC=DSIG4(P1,WGT,IMODE)  ! d u~ > vt~ ta- b~ e- e+ g b
        IF(IPROC.EQ.5) DSIGPROC=DSIG5(P1,WGT,IMODE)  ! d u~ > vt~ ta- b~ ve ve~ g b
        IF(IPROC.EQ.6) DSIGPROC=DSIG6(P1,WGT,IMODE)  ! d u~ > vt~ ta- b~ vt vt~ g b
      ENDIF
      RETURN
      END
